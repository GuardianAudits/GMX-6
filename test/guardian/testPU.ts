...
describe("Guardian.PositionUtils", () => {
  ...
  it("Capped PnL Typical Market", async () => {
    ...
    expect(await getBalanceOf(wnt.address, user0.address)).to.eq(
      profit.add(initialCollateral)
    );
  });
  ...
  it("Capped PnL Single Token Market", async () => {
    ...
    expect(await getBalanceOf(usdc.address, user0.address)).to.eq(
      profit.add(initialCollateral)
    );
  });
