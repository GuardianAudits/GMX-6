...
describe("Guardian.DecreasePositionCollateralUtils", () => {
  ...
  it("getEmptyFees and execution succeeds", async () => {
    ...
    const position0 = await reader.getPosition(
      dataStore.address,
      positionKeys[0]
    );
    ...
    expect(await getBalanceOf(wnt.address, user1.address)).to.eq(
      "8181818181818181819"
    );
    expect(await getBalanceOf(usdc.address, user1.address)).to.eq("0");
    // Verify Pool Amounts
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("991818181818181818181"); // 991.818181 ETH
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(1_020_000, 6));
  ...
  it("Collateral Delta Auto-Update: initialCollateralDelta > price impact diff", async () => {
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("991909090909090909090"); // 991.909 ETH
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(2_020_000, 6));
