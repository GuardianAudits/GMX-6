...
describe("Guardian.FundingFees", () => {
  ...
  it("Funding fees charged in a single token market are the same as those charged for a normal market", async () => {
    ...
    expect(
      await dataStore.getUint(
        keys.fundingUpdatedAtKey(ethUsdMarket.marketToken)
      )
    ).eq(0);

    await dataStore.setUint(
      keys.fundingFactorKey(ethUsdSingleTokenMarket.marketToken),
      decimalToFloat(5, 10)
    );
    await dataStore.setUint(
      keys.fundingExponentFactorKey(ethUsdSingleTokenMarket.marketToken),
      decimalToFloat(1)
    );

    expect(
      await dataStore.getUint(
        keys.fundingUpdatedAtKey(ethUsdSingleTokenMarket.marketToken)
      )
    ).eq(0);
    ...
    expect(normalMarketLongPositionInfo.position.addresses.market).to.eq(
      ethUsdMarket.marketToken
    );
    expect(normalMarketLongPositionInfo.position.flags.isLong).to.be.true;
    expect(normalMarketShortPositionInfo.position.addresses.market).to.eq(
      ethUsdMarket.marketToken
    );
    ...
    expect(
      normalMarketLongPositionInfo.fees.funding.fundingFeeAmount
    ).to.closeTo("40320334", "500"); // ~$40 of funding fees in USDC
    expect(normalMarketShortPositionInfo.fees.funding.fundingFeeAmount).to.eq(
      0
    );

    expect(
      normalMarketShortPositionInfo.fees.funding.claimableLongTokenAmount
    ).to.eq(0);
    expect(
      normalMarketShortPositionInfo.fees.funding.claimableShortTokenAmount
    ).to.closeTo("40320266", "500");

    expect(
      normalMarketLongPositionInfo.fees.funding.claimableLongTokenAmount
    ).to.eq(0);
    expect(
      normalMarketLongPositionInfo.fees.funding.claimableLongTokenAmount
    ).to.eq(0);

    expect(homogenousMarketLongPositionInfo.position.addresses.market).to.eq(
      ethUsdSingleTokenMarket.marketToken
    );
    expect(homogenousMarketLongPositionInfo.position.flags.isLong).to.be.true;
    expect(homogenousMarketShortPositionInfo.position.addresses.market).to.eq(
      ethUsdSingleTokenMarket.marketToken
    );
    ...
    expect(
      homogenousMarketLongPositionInfo.fees.funding.fundingFeeAmount
    ).to.closeTo("20160000", "500");
    expect(
      homogenousMarketShortPositionInfo.fees.funding.fundingFeeAmount
    ).to.eq("0");

    expect(
      homogenousMarketShortPositionInfo.fees.funding.claimableLongTokenAmount
    ).to.eq("10079999");
    expect(
      homogenousMarketShortPositionInfo.fees.funding.claimableShortTokenAmount
    ).to.eq("10079999");

    expect(
      homogenousMarketLongPositionInfo.fees.funding.claimableLongTokenAmount
    ).to.eq(0);
    expect(
      homogenousMarketLongPositionInfo.fees.funding.claimableShortTokenAmount
    ).to.eq(0);
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(positionFeesCollectedEvent.fundingFeeAmount).to.closeTo(
            "40320100",
            "500"
          );
          expect(positionFeesCollectedEvent.claimableLongTokenAmount).to.eq(
            "0"
          );
          expect(positionFeesCollectedEvent.claimableShortTokenAmount).to.eq(
            "0"
          );
        },
      },
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(positionFeesCollectedEvent.fundingFeeAmount).to.closeTo(
            "40320467",
            "500"
          ); // Slight differences due to timing
          expect(positionFeesCollectedEvent.claimableLongTokenAmount).to.eq(
            "0"
          );
          expect(positionFeesCollectedEvent.claimableShortTokenAmount).to.eq(
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(
            positionFeesCollectedEvent.claimableLongTokenAmount
          ).to.closeTo("20160049", "500");
          expect(
            positionFeesCollectedEvent.claimableShortTokenAmount
          ).to.closeTo("20160049", "500");
        },
      },
    });
    ...
    const currentClaimableFundingPerSizeLongUSDC = await dataStore.getUint(
      keys.claimableFundingAmountPerSizeKey(
        ethUsdMarket.marketToken,
        usdc.address,
        true
      )
    ...
    expect(position1.numbers.fundingFeeAmountPerSize).to.eq(
      currentFundingFeeAmountPerSizeLongUSDC
    );
  ...
  it("Funding fees match when paid in different directions", async function () {
    ...
    expect(balanceAfterUser2.sub(balanceBeforeUser2)).eq("25920028"); // $25.920028
    ...
    expect(WNTBalanceAfterUser1.sub(WNTBalanceBeforeUser1)).eq(
      "1728000600000000"
    ); // $8.640003
    ...
    expect(WNTBalanceAfterUser2.sub(WNTBalanceBeforeUser2)).eq(
      "1728000600000000"
    ); // $8.640003
