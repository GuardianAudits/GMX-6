...
describe("Guardian.EDU-1", () => {
  ...
  it("Invalid price impact while depositing into homogenous markets", async () => {
    ...
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdSingleTokenMarket.marketToken, true),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdSingleTokenMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.swapImpactExponentFactorKey(ethUsdSingleTokenMarket.marketToken),
      decimalToFloat(2, 0)
    );
    ...
    const user1MarketTokensAmount = await getBalanceOf(
    ...
    const user2MarketTokensAmount = await getBalanceOf(
      ethUsdSingleTokenMarket.marketToken,
      user2.address
    );
