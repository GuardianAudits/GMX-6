...
describe("Guardian.MKTU-1", () => {
  ...
  it("Borrowing fees can be skipped by manipulating OI", async () => {
    ...
    expect(
      await dataStore.getUint(
        keys.cumulativeBorrowingFactorUpdatedAtKey(
    ...
    expect(position0.fees.borrowing.borrowingFeeUsd).eq("0");
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expectWithinRange(
            positionFeesCollectedEvent.borrowingFeeAmount,
            "1728001365",
            "1728001366"
          );
          expect(positionFeesCollectedEvent.borrowingFeeReceiverFactor).eq(
            decimalToFloat(4, 1)
          );
          expectWithinRange(
