...
import { getBalanceOf, getSupplyOf,expectTokenBalanceIncrease } from "../../utils/token";
...
describe("THE POCER", () => {
  ...
  let dataStore,exchangeRouter, ethUsdMarket,ethEthEthMarket, referralStorage, wnt, usdc, ethUsdSingleTokenMarket, reader;
  ...
  beforeEach(async () => {
    ...
    ({ dataStore,exchangeRouter, ethUsdMarket, referralStorage, wnt, usdc,ethUsdSingleTokenMarket, reader} = fixture.contracts);
...
// LONGS PAYS SHORTS
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
            ...
            expect(positionFeesCollectedEvent.fundingFeeAmount).eq("2400400000000"); // 0.0000024004 ETH
            expect(positionFeesCollectedEvent.borrowingFeeAmount).eq("2088007649694012"); // 0.0020880 ETH            
        },
      },
    });
    ...
    await handleOrder(fixture, {
        ...
        execute: {
          afterExecution: ({ logs }) => {
              ...
              const positionFeesCollectedEvent = getEventData(logs, "PositionFeesCollected");
              expect(positionFeesCollectedEvent.fundingFeeAmount).eq("4124053246754"); // 0.000004124053 ETH
              expect(positionFeesCollectedEvent.borrowingFeeAmount).eq("1007772937432147"); // 0.00100777 ETH            
          },
        },
      });
  ...
  expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq("50000000000000000000000");
    ...
    await handleDeposit(fixture, {
      ...
      expect(await getBalanceOf(ethUsdMarket.marketToken, user2.address)).eq("9999254451370059838522");
  ...
  await handleOrder(fixture, {
    ...
    await handleOrder(fixture, {
      ...
      await handleOrder(fixture, {
        ...
        expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq("99962376511838209303154");
        ...
        expect(await getBalanceOf(ethUsdMarket.marketToken, user1.address)).eq("49997999840059193216263");
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
            ...
            const positionFeesCollectedEvent = getEventData(logs, "PositionFeesCollected");
            expect(positionFeesCollectedEvent.fundingFeeAmount).eq("2400400000000"); // 0.0000024004 ETH
            expect(positionFeesCollectedEvent.borrowingFeeAmount).eq("2109080741539804"); // 0.0021090 ETH            
        },
      },
    });
    ...
    await handleOrder(fixture, {
        ...
        execute: {
          afterExecution: ({ logs }) => {
              ...
              const positionFeesCollectedEvent = getEventData(logs, "PositionFeesCollected");
              expect(positionFeesCollectedEvent.fundingFeeAmount).eq("4124053246754"); // 0.000004124053 ETH
              expect(positionFeesCollectedEvent.borrowingFeeAmount).eq("1017942331581406"); // 0.0010179 ETH            
          },
        },
      });
  ...
  await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
            ...
            const positionFeesCollectedEvent = getEventData(logs, "PositionFeesCollected");
            expect(positionFeesCollectedEvent.fundingFeeAmount).eq("4147488000000"); // 0.00000414 ETH
            expect(positionFeesCollectedEvent.borrowingFeeAmount).eq("6233005122999151"); // 0.0062330 ETH
        },
      },
    });
    ...
    await handleOrder(fixture, {
        ...
        execute: {
          afterExecution: ({ logs }) => {
              ...
              expect(positionFeesCollectedEvent.borrowingFeeAmount).eq("1321781174149024"); // 0.0013217 ETH
          },
        },
      });
      ...
      await handleOrder(fixture, {
        ...
        expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq("99961911135069028995802");
  ...
  await handleOrder(fixture, {
    ...
    await handleOrder(fixture, {
      ...
      await handleOrder(fixture, {
        ...
        expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq("99963310773094790924599");
