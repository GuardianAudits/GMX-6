...
describe("Guardian.Fees", () => {
  ...
  it("FeeKeeper claims fees through FeeHandler", async () => {
    ...
    claimableFeeAmount = await dataStore.getUint(
      keys.claimableFeeAmountKey(ethUsdMarket.marketToken, usdc.address)
    );
    ...
    expect(claimableFeeAmount).to.eq(
      positionFeeFromDecrease.add(positionFeeFromIncrease)
    );
    ...
    expect(claimableFeeAmount).to.eq(
      positionFeeFromDecrease.mul(2).add(positionFeeFromIncrease)
    );
    ...
    await feeHandler
      .connect(wallet)
      .claimFees([ethUsdMarket.marketToken], [usdc.address]);
  ...
  it("Affiliates are able to claim their rewards", async () => {
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(positionFeesCollectedEvent.positionFeeAmount).to.eq(
            expandDecimals(250, 6)
          );

          // Discounted fee is $200
          expect(positionFeesCollectedEvent.protocolFeeAmount).to.eq(
            expandDecimals(200, 6)
          );

          // Trader splits $50 discount with the affiliate
          expect(positionFeesCollectedEvent.affiliate).to.eq(user1.address);
          expect(positionFeesCollectedEvent.totalRebateAmount).to.eq(
            expandDecimals(50, 6)
          );
          expect(positionFeesCollectedEvent.traderDiscountAmount).to.eq(
            expandDecimals(25, 6)
          );
          expect(positionFeesCollectedEvent.affiliateRewardAmount).to.eq(
            expandDecimals(25, 6)
          );
        },
      },
    ...
    expect(affiliateReward).to.eq(
      affiliateRewardsFromDecrease.add(affiliateRewardsFromIncrease)
    );
    ...
    expect(affiliateReward).to.eq(
      affiliateRewardsFromDecrease.mul(2).add(affiliateRewardsFromIncrease)
    );
  ...
  it("UI fees are claimable", async () => {
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          // .005 * 50,000 = $250
          expect(positionFeesCollectedEvent.uiFeeAmount).to.eq(
            expandDecimals(250, 6)
          );
        },
      },
    ...
    expect(claimableUIFees).to.eq(
      uiFeesFromIncrease.add(uiFeesFromDecrease.mul(2))
    );
    ...
    await exchangeRouter
      .connect(user1)
      .claimUiFees([ethUsdMarket.marketToken], [usdc.address], user1.address);
